...
export const AddToWishList = async (req, res) => {
  ...
  try {
    if (id === userId) {
      const existedWishListProduct = await WishListModel.findOne({
        userId,
        productId,
      });
      ...
      if (!existedWishListProduct) {
        ...
        return res
          .status(403)
          .json({ message: "Product already existed in wish list" });
      }
    } else {
...
export const GetWishListItems = async (req, res) => {
  ...
  try {
    if (id === userId) {
      ...
      return res
        .status(200)
        .json({ message: "Wishlist fetched successfully", wishList: products });
    } else {
      return res.status(401).json({ message: "Unauthorized access" });
    }
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "Internal server error during fetching the wish list" });
  }
};
...
export const removeFromWishList = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        message: "Internal server error during removing from the wish list",
      });
  }
};
